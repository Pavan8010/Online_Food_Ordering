    ...
    switch (action.type) {
        ...
        case ADD_TO_FAVORITE_SUCCESS:
            return {...state,
                isLoading:false,
                error:null,
                favorites:isPresentInFavorites(state.favorites,action.payload) // check if present in favorites or not
                ? state.favorites.filter((item)=>item.id!==action.payload.id) // remove item if already exists in favorites
                : [action.payload,...state.favorites]  // if not present then add to favorites
            }
